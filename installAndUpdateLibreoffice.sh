...
then
  ...
  # New version
  NEW_VERSION=`${CURL} -s  https://www.libreoffice.org/download/download/ | egrep "dl_version_number" | head -1 | sed 's@</span.*$@@' | sed 's@^.*number">@@'`
  ...
  update() {
  	...
  	do
  	echo "Testando mirror ${MIRROR}"
  	${CURL} --silent --head --fail ${MIRROR}/${NEW_VERSION}/${PACKAGE}/${ARCH//-/_}/ &> /dev/null
  	if [ $? = 0 ]
  	then
  		echo "O mirror $MIRROR está on-line e o dowload será feito por ele."
  		...
  		then
  			...
  			cd /tmp
  			echo "Criando o diretório temporário para salvar os arquivos"
  		...
  		echo "O mirror $MIRROR está offline, testando o próximo"
  	fi
  done
  }
